...
The two URLs are also known as `root` URLs. One of the two servers is supposed to exist and have a valid certificate. If this is not possible, but you know or can discover by other means the `root` URL(s), they can be given in the `origins` parameter:
...
You can use the passive authentication if your onprem server has ADFS configured. To use this auth set the `auth` param:
