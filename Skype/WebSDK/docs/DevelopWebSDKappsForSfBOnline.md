...
>**Note:** If you are unable to troubleshoot an issue with the SDK and need to submit a troubleshooting request, make sure to include the **correlationIds.sessionId** parameter when initializing the SDK as above, and the **version** parameter when calling **signInManager.signIn** of the form **{Your-App-Name}/{version-number}** (see below). The **sessionId** parameter should be generated by your client code and should be unique per session, so that if you want to ask what went wrong during a particular session, you can provide us the unique session ID and version to allow us to locate records specific to your session in our telemetry.
