...
The steps shown here assume that you have already created an application and have received a response that contains the HREF for an [application (UCWA)](application_ref.md) resource. For more information, see[Create an application](CreateAnApplication.md).

1. Send a POST request on the  **makeMeAvailable** resource.
    
    A POST request on the [makeMeAvailable (UCWA)](makeMeAvailable_ref.md) resource makes it possible for the user of the application to receive incoming messages. In the following sample, the request body contains information about the supported modality, messaging.
...
3. Process the response from the previous GET request.
    ...
    The response from a GET request on the [application (UCWA)](application_ref.md) resource contains useful links, as well as important embedded resources:[me (UCWA)](me_ref.md), [people (UCWA)](people_ref.md), [onlineMeetings (UCWA)](onlineMeetings_ref.md), and [communication (UCWA)](communication_ref.md).
...
6. Send a GET request on the  **events** resource.
    ...
    In the interest of brevity, only this GET request on the [events (UCWA)](events_ref.md) resource is shown. Subsequent GET requests on the **events** resource are similar and differ only in the query parameter (ack = _n_ ). An application normally will send a series of GET requests on this resource.
...
7. Process the response from the previous GET request.
    ...
    The events shown here come from two senders: the [me (UCWA)](me_ref.md) resource and the[communication (UCWA)](communication_ref.md) resource. For a given sender there are links to a number of events, along with the type of event, such as "updated" or "added".
...
8. Process the response from the next GET request on the  **events** resource.
    ...
    In this sample we see that the sender is the [communication (UCWA)](communication_ref.md) resource. The event channel contains several embedded resources, including a[messagingInvitation (UCWA)](messagingInvitation_ref.md) resource that has been started. This and other embedded resources indicate that an incoming message from Lene Aaling will arrive soon. The invitation contains links to the[accept (UCWA)](accept_ref.md) and[decline (UCWA)](decline_ref.md) resources, one of which can be used to accept or decline the incoming message. Notice that the[message (UCWA)](message_ref.md) link contains the actual text of the message.
...
9. Process the response from the next GET request on the  **events** resource.
    ...
    n this sample, the [conversation (UCWA)](conversation_ref.md) resource is the event sender, indicating that a remote participant (Lene Aaling) has been added to the conversation.
...
10. Process the response from the next GET request on the  **events** resource.
    ...
    Notice that the href in the [message (UCWA)](message_ref.md) link contains the text of the message as a Data URL.
...
11. Send a GET request on the [participant (UCWA)](participant_ref.md) resource, the resource for Lene Aaling.
...
13. Send a POST request on the  **accept** resource.
    ...
    The [accept (UCWA)](accept_ref.md) resource, which can be found in the cache, was shown in step 8.
