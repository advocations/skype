...
1. Send a POST request on the [startMessaging (UCWA)](startMessaging_ref.md) resource, which can be found in the[communication (UCWA)](communication_ref.md) resource embedded in **application**. A POST request on this resource causes a [messagingInvitation (UCWA)](messagingInvitation_ref.md) resource to be created and started in the event channel.
...
3. Send a GET request on the [events (UCWA)](events_ref.md) resource.
...
4. Process the response from the previous request. 
    ...
    The  **messagingInvitation** is returned as an embedded resource. Another embedded resource that is the[conversation (UCWA)](conversation_ref.md) resource, which is used in the next step. Other information contained here is that the **conversation** is in the Connecting state.
...
9. Send a POST request on the [sendMessage (UCWA)](sendMessage_ref.md) resource embedded in the **conversation**.
...
13. Send a POST request on the [stopMessaging (UCWA)](stopMessaging_ref.md) href.
