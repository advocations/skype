...
10. Process the response from the previous request. 
    ...
    The response from the previous POST request should be 201 Created, which indicates that your application is now registered with the UCWA 2.0 server. The body of the response contains the application resource, which contains basic information about the application, links to related resources such as the [batch (UCWA)](batch_ref.md) and[events (UCWA)](events_ref.md) resources, and four resources that are so useful that they are embedded in the application resource -[me (UCWA)](me_ref.md), [people (UCWA)](people_ref.md), [onlineMeetings (UCWA)](onlineMeetings_ref.md), and [communication (UCWA)](communication_ref.md). 
